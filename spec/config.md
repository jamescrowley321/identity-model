...
| `http.retry.max_delay` | `HTTP_RETRY_MAX_DELAY` | float secs | `120.0` | > 0 | py: ceiling on a single retry wait, applied to the computed backoff on both the 429/5xx path and the connection-error path. A default, not a hard limit — a deployment that wants longer waits raises it; invalid or non-positive values log a warning and fall back to the default. No upper bound: the sync client sleeps in slices, so a wait longer than `time.sleep()` accepts in one call is still executed | ✔ | n/a | n/a |
| `http.retry.base_delay` | `HTTP_RETRY_BASE_DELAY` | float secs | `1.0` | ≥ 0 | py: invalid, non-finite or negative values log a warning and fall back to the default. The computed backoff is bounded by `http.retry.max_delay` | ✔ | n/a | n/a |
...
`config.json` is currently a **prose contract** (no executable `vectors`), like most capability files: each language's implementation proves it with its own tests keyed to the `CFG-1xx` IDs. Promoting these cases to executable vectors means adding a `(language, capability)` entry to `tools/spec_coverage_gate.py`'s `RUNNERS` for every language in the same change: the gate already handles per-capability reports, and it hard-fails on a capability that carries executable vectors but has no runner, so partial promotion cannot silently go ungated.
